...
self.addEventListener('fetch', (event) => {
  ...
  // Solo GET (no tocamos POST/PUT/etc)
  if (request.method !== 'GET') return;

  const url = new URL(request.url);

  // 1) APP SHELL (archivos locales principales)
  ...
  // 2) Recursos de CDN dinámicos (Bootstrap, PouchDB)
    ...
    event.respondWith(
      caches.match(request).then((cached) => {
        ...
        return fetch(request)
          ...
          .catch(() => {
            // Último recurso: intenta por pathname sin query
  ...
  // 4) Otros GET: network-first con fallback a cache
  event.respondWith(
    fetch(request)
      .then((res) => res)
